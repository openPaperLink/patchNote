...
//		if (type == 17) return true; // ConstructorInvocation
	...
	public static boolean isComment(int type) {
		if(type == 63) return true; // LineComment
		if(type == 64) return true; // BlockComment
		return false;
	}
	...
	public static boolean isMethodReference(int type) {
		if (type == 32) return true; // Method Invocation
		if (type == 48) return true; // Super Method Invocation
		return false;
	}



	public static boolean isMethodOrConstructorInvocation(int type) {
		return isMethodInvocation(type) || isConstructorInvocation(type)
				|| isSuperConstructorInvocation(type) || isSuperMethodInvocation(type);
	}

	public static boolean isVariableDeclaration(int type) {
	...
	public static boolean isLiteral(int type){
		return isNullLiteral(type) || isCharacterLiteral(type) || isStringLiteral(type) || isBooleanLiteral(type)
				|| isTypeLiteral(type) || isNumberLiteral(type);
	}
	//这些节点类型不需要处理
	public static boolean isNohandle(int type){
		//TODO:需要补充
		return type == 1 ||type == 73 || type == 74 || type == 75 || type == 76 || type == 81 || type == 82 ||type == 83 ||type == 84 ||type == 87 ||
				type == 88 ||type == 71 ||type == 72||type == 52 ||type == 55 || type == 56 || type == 57 || type == 39 || type == 43 || type == 5 ;
