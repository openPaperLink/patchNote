...
public class ExpJdtVisitor extends JdtVisitor {
	...
	public boolean visit(EnumDeclaration node) {
    	...
    	push(node,node.getNodeType(), node.getClass().getSimpleName(), label, startPosition, nodeStartPosition + length - startPosition);
        return true;
	}
	...
	public boolean visit(Assignment node) {
		...
		push(node,-1, "Operator", op, leftHandExp.getStartPosition() + leftHandExp.getLength(), op.length());
		popNode();
...
//		}
		...
		push(node,-2, "New", "new", type.getStartPosition() - 4, 3);
		popNode();
		type.accept(this);
		visitList(arguments);
	...
	public boolean visit(InfixExpression node) {
		...
		String op = node.getOperator().toString();
		push(node,-1, "Operator", op, leftExp.getStartPosition() + leftExp.getLength(), op.length());
		popNode();
	...
	public boolean visit(InstanceofExpression node) {
		...
		exp.accept(this);
		push(node,-3, "Instanceof", "instanceof", exp.getStartPosition() + exp.getLength() + 1, 10);
		popNode();
...
//				popNode();
//			}
			push(methodName,42, "SimpleName", "MethodName:" + methodName.getFullyQualifiedName() + ":" + arguments.toString(),
...
//		}
	...
	public boolean visit(PostfixExpression node) {
		...
		exp.accept(this);
		String op = node.getOperator().toString();
		push(node,-1, "Operator", op, exp.getStartPosition() + exp.getLength() + 1, op.length());
		popNode();
		return false;
	}
	...
	public boolean visit(PrefixExpression node) {
		pushNode(node, node.toString());
		String op = node.getOperator().toString();
		push(node,-1, "Operator", op, node.getStartPosition(), op.length());
		popNode();
		Expression exp = node.getOperand();
		exp.accept(this);
		return false;
...
//        pushNode(node, nodeStr);
    	push(node,node.getNodeType(), node.getClass().getSimpleName(), nodeStr, startPosition, nodeStartPosition + length - startPosition);
...
//		push(node,node.getNodeType(), node.getClass().getSimpleName(), methodLabel, startPosition, nodeStartPosition + length - startPosition);
...
//        visitBody(node.getBody());
    ...
    public boolean visit(IfStatement node) {
        ...
        if (stmt != null) {
        	push(stmt,8, "Block", "ThenBody", stmt.getStartPosition(), stmt.getLength());
            visitBody(stmt);
            popNode();
        }
        ...
        if (stmt != null) {
        	push(stmt,8, "Block", "ElseBody", stmt.getStartPosition(), stmt.getLength());
            visitBody(stmt);
            popNode();
        }
        return false;
...
//        visitBody(node.getBody());
        ...
        if (stmt != null) {
        	push(stmt,8, "Block", "FinallyBody", stmt.getStartPosition(), stmt.getLength());
            visitBody(stmt);
            popNode();
        }
        return false;
